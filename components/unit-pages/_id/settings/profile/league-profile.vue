      ...
      <div class="w-full flex flex-wrap mt-8">
        ...
        <div class="w-full sm:w-1/2 pr-3 sm:pr-12 flex mt-8 sm:mt-0">
          ...
          <div class="w-full flex flex-col ml-4">
            <data-label :label="$t('app.team.settings.profile.your_league_account')">
              {{ $get(unit, 'entity.name') }}
            </data-label>
            <app-button class="text-left pt-4" data-cy="go-to-profile" :title="$t('app.team.settings.profile.go_to_league_profile')" :btn-class="'link'" @clicked="goToUnitProfile()" />
          </div>
        </div>
